# type: ignore
# Adapted from https://github.com/patrickvonplaten/controlnet_aux/blob/master/src/controlnet_aux/lineart/__init__.py
import os
import warnings
...
class LineartDetector:
    ...
    @classmethod
    def from_pretrained(cls, model_path, coarse_model_path):
        assert os.path.exists(model_path), "model path must be accessible"
        assert os.path.exists(coarse_model_path), "coarse model path must be accessible"
    ...
    def __call__(
        self, input_image, coarse=False, detect_resolution=512, image_resolution=512, output_type="pil", **kwargs
    ):
